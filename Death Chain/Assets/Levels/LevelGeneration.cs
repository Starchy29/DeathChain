...
public class LevelGeneration : MonoBehaviour
{
    ...
    // TEMP
    public List<GameObject> tempChunks;
    ...
    void Start()
    {
        //Generate(5);
        Timer.ClearTimers();
        TempGenerate(); // for maker pitch
    }

    private void TempGenerate() {
        GameObject currentChunk = Instantiate(tempChunks[0]);
        ...
        for(int i = 1; i < tempChunks.Count; i++) {
            GameObject newChunk = Instantiate(tempChunks[i]);
            chunks.Add(newChunk);
        ...
        // position end zone (attached to this)
        Transform endGate = chunks[chunks.Count - 1].transform.GetChild(1);
        transform.position = endGate.position + new Vector3(0, 2.5f, 0);
